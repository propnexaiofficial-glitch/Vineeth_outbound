...
from routers_call_details import router as call_details_router
from outbound_call import router as outbound_call_router
from bonvoice_call_initiate import router as bonvoice_router, CALL_CONTEXT_STORE
from call_data_store import save_call_data
# ─────────────────────────────────────────────────────────────────────────────
...
async def voice_agent_ws(websocket: WebSocket):
    ...
    # ── Bonvoice outbound call context lookup ───────────────────────────
    # When this call was triggered via POST /api/call/initiate, Bonvoice's
    # voicebotURL for this call carries a short `call_reference` query
    # param (see bonvoice_call_initiate.py — the full lead context can't
    # go in the URL itself due to Bonvoice's 255-char column limit).
    # We look that reference up here to get everything SchoolKnot sent us
    # (parent_name, student_name, grade, branch_name, appointment details,
    # prompt_type, etc.) and hand it to WsCallHandler so the agent can
    # actually use it when greeting the caller.
    call_reference = qp.get("call_reference", "")
    outbound_context: dict = {}

    if call_reference:
        outbound_context = CALL_CONTEXT_STORE.get(call_reference, {}) or {}
        if outbound_context:
            logger.info(
                f"[{call_reference}] Loaded outbound call context for this call: "
                f"{outbound_context}"
            )
        else:
            # Most likely cause: the server restarted between
            # /api/call/initiate being called and this call connecting
            # (CALL_CONTEXT_STORE is in-memory only — see the note in
            # bonvoice_call_initiate.py about moving this to Postgres/Redis
            # for production robustness).
            logger.warning(
                f"[{call_reference}] call_reference present on the WS "
                f"connection but no matching context found in "
                f"CALL_CONTEXT_STORE — agent will fall back to generic "
                f"query-param values only."
            )

    handler = WsCallHandler(
        websocket,
        on_call_end=_on_call_end,
        lead_name=outbound_context.get("parent_name") or qp.get("lead_name", "there"),
        lead_company=qp.get("lead_company", ""),
        prompt_type=outbound_context.get("prompt_type") or qp.get("prompt_type", "sales"),
        call_context=qp.get("call_context", ""),
        is_outbound=bool(call_reference) or qp.get("is_outbound", "false").lower() == "true",
